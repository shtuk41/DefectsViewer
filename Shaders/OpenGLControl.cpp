...
bool COpenGLControl::InitOpenGL(HINSTANCE hInstance, HWND* a_hWnd, int iMajorVersion, int iMinorVersion, void (*a_InitScene)(LPVOID), void (*a_RenderScene)(LPVOID), void(*a_ReleaseScene)(LPVOID), void(*a_ReloadScene)(LPVOID), LPVOID lpParam)
{
	...
	if(bError)
	{
		...
		MessageBox(*hWnd, (LPCWSTR)sErrorMessage, (LPCWSTR)sErrorTitle, MB_ICONINFORMATION);
		return false;
	}
...
Result:	Calls reload scene

/*---------------------------------------------*/
void COpenGLControl::Reload(LPVOID lpParam)
{
	if (ReloadScene)
	{
		ReloadScene(lpParam);
